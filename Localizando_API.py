...
# -------------------------------
# Aparência: seletor de tamanho e cor dos marcadores
# -------------------------------
...
client_color = color_map[color_option]
cd_color = [220, 0, 0]  # vermelho fixo
...
# Título com marcador padrão (pin clássico)
st.markdown("""
<h1 style='display: flex; align-items: center;'>
  <img src='https://upload.wikimedia.org/wikipedia/commons/e/e3/Location_dot_red.svg' width='24' style='margin-right: 10px;'/>
...
if arquivo:
    ...
    # Pontos válidos com ícone e cor
    pontos = [
        {
            "lat": r["Latitude"],
            "lon": r["Longitude"],
    ...
    if cd_lat is not None and cd_lon is not None and pontos_ordenados_rota:
        ...
        cd_point = {"lat": cd_lat, "lon": cd_lon, "name": "Centro de Distribuição", "icon": "marker", "color": cd_color}

        # Camada de ícones (marcadores padrão)
        icon_layer = pdk.Layer(
            "IconLayer",
            data=pontos_ordenados + [cd_point],
            get_icon="icon",
            get_size=icon_size,
            ...
            icon_mapping={
                "marker": {
                    "x": 0, "y": 0, "width": 128, "height": 128, "anchorY": 128, "anchorX": 64
                }
            }
        )
        ...
        st.pydeck_chart(pdk.Deck(layers=[icon_layer, path_layer], initial_view_state=view_state, tooltip={"text": "{name}"}))
