...
# -------------------------------
# Rota real com ORS (CD -> clientes na ordem fornecida) + resumo
# -------------------------------
def gerar_rota_real(cd_lat, cd_lon, pontos):
    ...
    try:
        ...
        resumo = rota['features'][0]['properties']['summary']  # distance (m) e duration (s)
...
def calcular_tempo_distancia(cd_lat, cd_lon, pontos, max_clientes=50):
    ...
    # Limitar para proteger chave ORS
    ...
    for p in pontos_avaliar:
        try:
            ...
            resultados.append({"Cliente": p["name"], "Tempo (min)": None, "Distância (km)": None})
        time.sleep(0.3)  # suaviza taxa de chamadas
    return pd.DataFrame(resultados)
...
if arquivo:
    ...
    # ✏️ Inserção manual de coordenadas (um campo só)
    ...
    # Construção da lista de pontos válidos
    pontos = [
        {
            "lat": r["Latitude"],
            "lon": r["Longitude"],
    ...
    # Tabela tempo/distância por cliente (mantida e protegida)
    df_td = pd.DataFrame()
    ...
    # Limite opcional para rota (proteção de chave ORS)
    max_pontos_rota = 50
