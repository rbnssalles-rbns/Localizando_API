...
if arquivo:
    ...
    # Pontos válidos com ícone (URL) e nome
    pontos = [
        {
            "lat": r["Latitude"],
            "lon": r["Longitude"],
            "name": f"{r['Cliente_ID']} - {r['Cliente']}" if "Cliente" in df.columns else str(r["Cliente_ID"]),
            # objeto de ícone por ponto (definindo URL para a cor escolhida)
            "icon_data": {
                "url": client_marker_url,
                "width": 25, "height": 41, "anchorX": 12, "anchorY": 41
            }
        }
    ...
    if cd_lat is not None and cd_lon is not None and pontos_ordenados_rota:
        ...
        # Ponto do CD com ícone vermelho fixo via URL
        cd_point = {
            "lat": cd_lat,
            "lon": cd_lon,
        ...
        # Camada de ícones (pins coloridos via URL)
        icon_layer = pdk.Layer(
            "IconLayer",
            data=pontos_ordenados + [cd_point],
            get_icon="icon_data",              # acessa o campo por ponto
            get_size=icon_size,                # slider
            size_scale=size_scale,             # slider
