...
# -------------------------------
API_KEY_ORS = "COLOQUE_SUA_CHAVE_AQUI"
try:
...
# -------------------------------
# Funções de rota com ORS
# -------------------------------
def gerar_rota(cd_lat, cd_lon, pontos, optimize=None):
...
if arquivo:
    ...
    pontos = [
        {
            "lat": r["Latitude"],
            "lon": r["Longitude"],
            "name": f"{r['Cliente_ID']} - {r.get('Cliente','')}"
        }
    ...
    opcao = st.sidebar.radio("Escolha uma simulação:", ["Rota 1 - Distância", "Rota 2 - Tempo", "Rota 3 - Ordem Original"])

    if cd_lat is not None and cd_lon is not None:
        st.subheader("🗺️ Mapa de clientes e rota simulada")

        mapa = folium.Map(location=[cd_lat, cd_lon], zoom_start=11)

        # Marcador do CD
        folium.Marker(
            location=[cd_lat, cd_lon],
        ...
        # Rota conforme cenário
        if opcao == "Rota 1 - Distância":
            rota = gerar_rota(cd_lat, cd_lon, pontos, optimize="distance")
        elif opcao == "Rota 2 - Tempo":
            rota = gerar_rota(cd_lat, cd_lon, pontos, optimize="duration")
        else:
            rota = gerar_rota(cd_lat, cd_lon, pontos)
